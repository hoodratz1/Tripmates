...
app.post('/login', function (req, res) {
  sess = req.session;
  !sess.users ? sess.users = [] : null;
  var currentUser = '';
  console.log(sess.users)
  sess.users.forEach(user => user.ip === req.connection.remoteAddress ? currentUser = user.email : null) 
...
app.get('/logout',function(req,res){
  sess = req.session;
  sess.users.forEach(user => user.ip === req.connection.remoteAddress ? user.ip = null : null) ;
  res.redirect('/');
});
...
app.get('/yelp', function (req, res) {
  ...
  var resultArray = [];
  var tooManyQueriesResponse = [{name: 'Too many API queries. You have been blocked', url: 'too many queries!!', rating: 0, image_url: 'https://m.popkey.co/8d4819/rbGY7.gif'}] 
